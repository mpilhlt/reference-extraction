...
<xsl:stylesheet
  ...
  <xsl:function name="local:tag">
    ...
    <xsl:text>(</xsl:text><xsl:value-of select="string-length(string-join($node/preceding::text()[ancestor::tei:text]))"/>
    <xsl:text>, </xsl:text><xsl:value-of select="string-length(string-join($node/preceding::text()[ancestor::tei:text])) + string-length(string-join($node//text()))"/>
  ...
  "text": "<xsl:value-of select="translate($body, '&#xA;', ' ')"/>",
